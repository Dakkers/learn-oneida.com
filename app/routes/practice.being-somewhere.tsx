...
import { QuizPage, QuizPageProps } from "~/components/practice/QuizPage";
...
export default function PracticePlaces() {
  const data: QuizPageProps["data"] = React.useMemo(() => {
    const result: QuizPageProps["data"] = [];
    for (const datum of [
      dataIsHere,
      dataWasHere,
      dataWillBeHere,
      ...
      for (const phrase of datum.phrases) {
        result.push({
          en: translatePhrase(datum.translation, phrase.pronoun),
          key: `${datum.translation}_${phrase.pronoun}`,
          on: convertBreakdownToPlainText(phrase.breakdown, {
            suffix: datum.suffix,
          }),
        });
      }
